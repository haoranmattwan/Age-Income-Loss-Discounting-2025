...
The raw dataset is loaded and undergoes several processing steps to prepare for analysis. This includes filtering to the final sample (N = 594), calculating individual-level AuC values, and creating scaled predictor variables for the regression models.
...
Disc_Raw <- read_excel("Supplementary Data.xlsx", sheet = 2) |>
  mutate(age_grp_factor = factor(case_when(Age >= 20 & Age < 35 ~ "20-34",
                                           Age >= 35 & Age <= 50 ~ "35-50",
                                           Age > 50 & Age < 65 ~ "51-64",
                                           Age >= 65 & Age <= 80 ~ "65-80"),                                   
  ...
  mutate(across(.cols = c(Income, Education, Distress, Anxiety, Depression), ~as.numeric(.)))

# --- Create Group-Level Data Frame for Plotting (Figure 1) ---
Disc_Grp <- Disc_Raw |>
  mutate(Amount = factor(if_else(Amount==150, "$150", if_else(Amount==30000, "$30,000", "$2,500")), 
                         levels = c("$150", "$2,500", "$30,000"))) |>
  group_by(age_grp_factor, Amount, Delay) |> 
...
AuC_ID <- Disc_Raw |>
  ...
  summarise(AuC = auc(Delay/max(Delay), RSV), .groups = 'drop') |>
  # Join back the unique demographic info for each participant
  left_join(distinct(Disc_Raw, ID, Age, age_grp_factor, Income, Gender, Education, 
...
Disc_Grp |>
  arrange(desc(Amount)) |>
  ggplot(aes(Delay, Mean_RSV, shape = Amount, fill = Amount, color = Amount, linetype = Amount)) +
  ...
  facet_wrap(~age_grp_factor, nrow = 1) +
...
Disc_Grp |>
  group_by(age_grp_factor, Amount) |>
  reframe(R2 = rsquare(nlsLM(Mean_RSV ~ 1 / (1 + k * Delay)^s, 
                             data = pick(everything()), start = list(k = .1, s = 1)), 
                       data = pick(everything()))) |>
  pivot_wider(names_from = age_grp_factor, values_from = R2) |> 
  print(digits = 4)
...
AuC_ID |>
  ...
  pivot_wider(names_from = age_grp_factor, values_from = p_val) |>
  print(digits = 4)
...
DL_mod1b <- betaMLM_brm(Formula = bf(AuC ~ Age_std + I(Age_std^2) + (1 | ID)), 
                        Data = DL_dat1, File = "rds/DL_mod1b")
...
DL_modS1 <- betaMLM_brm(Formula = bf(AuC ~ Age_std + I(Age_std^2) + Income_std + Depression_std + (1|ID)), 
                        Data = DL_dat3, File = "rds/DL_modS1")
...
DL_modS2 <- betaMLM_brm(
  ...
  Data = DL_dat4, File = "rds/DL_modS2"
)
...
DL_modS3 <- betaMLM_brm(Formula = bf(AuC ~ Age_std + I(Age_std^2) + Income_std + Distress_std + (1|ID)), 
                        Data = DL_dat3, File = "rds/DL_modS3")
...
DL_modS4 <- betaMLM_brm(
  ...
  Data = DL_dat4, File = "rds/DL_modS4"
)
...
pred_grid_amount <- expand(DL_dat4, Age = seq(35, 80, 1),
  ...
  mutate(
    ...
    Anxiety_std = 0, Health_std = 0,
    Amount_Factor = factor(Amount, levels = c(150, 2500, 30000), labels = c("$150", "$2,500", "$30,000"))
  )
